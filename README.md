...
当前前端为 React + Vite，后端为 FastAPI + PostgreSQL，推荐通过 Docker Compose 部署。
...
- MoviePilot / Twilight：订阅同步、资源搜索、下载推送、缺集预览与精准补齐。
...
数据库默认使用 Compose 内置 PostgreSQL，数据保存在 Docker volume：

```text
postgres-data
```

应用主数据库只支持 PostgreSQL。后端启动时会通过 Alembic 执行版本化 schema 迁移；`data/` 目录仍用于运行时配置、本地 TMDB 资料库和其它文件型缓存。
...
MoviePilot / Twilight 集成用于把 PT 下载链路接入 MediaSync115：

- 支持创建 MoviePilot 订阅并保存外部订阅 ID。
- 同一媒体可以同时存在 115 订阅和 MoviePilot 订阅，两条通道互不改写。
- MoviePilot TV 订阅可以查看 MediaSync115 计算出的缺集状态，但不会进入 115 自动转存流程。
- 支持“预览补缺”和“用 MoviePilot 补缺”：系统会搜索 MoviePilot 资源、解析集数，并只自动推送明确匹配的单集资源。
- 季包、全集包、多集包、标题或年份不明确的资源会进入需人工确认/跳过状态，避免误推送。
- 设置页可启用 MoviePilot 定时同步，定期同步订阅、下载和转移状态。
...
上述正式部署 Compose 均包含 PostgreSQL 服务，应用通过 `DATABASE_URL=postgresql+asyncpg://...` 连接数据库。
